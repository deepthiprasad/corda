...
interface VaultService {
     ...
     */
    fun <T : ContractState> states(clazz: Class<T>, statuses: EnumSet<Vault.StateStatus>): List<StateAndRef<T>>
}

inline fun <reified T: ContractState> VaultService.unconsumedStates(): List<StateAndRef<T>> =
        states(T::class.java, EnumSet.of(Vault.StateStatus.UNCONSUMED))

inline fun <reified T: ContractState> VaultService.consumedStates(): List<StateAndRef<T>> =
        states(T::class.java, EnumSet.of(Vault.StateStatus.CONSUMED))
