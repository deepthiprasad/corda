...
class NodeVaultService(private val services: ServiceHub, dataSourceProperties: Properties) : SingletonSerializeAsToken(), VaultService {
    ...
    override fun <T: ContractState> states(clazz: Class<T>, statuses: EnumSet<Vault.StateStatus>): List<StateAndRef<T>> {
        val stateAndRefs =
            session.withTransaction(TransactionIsolation.REPEATABLE_READ) {
    ...
    private fun makeUpdate(tx: WireTransaction, netDelta: Vault.Update, ourKeys: Set<PublicKey>): Vault.Update {
        ...
        // Calculate the states that are being spent by this transaction.
        val txnConsumedStates = tx.inputs.map {
            val state = services.loadState(it)
            StateAndRef(state, it)
        }.toHashSet()

        // Retrieve existing consumed states of same type
        val unconsumedStatesOfType =
            if (txnConsumedStates.isNotEmpty()) {
                val stateType = txnConsumedStates.first().state.data.javaClass
                states(stateType, EnumSet.of(Vault.StateStatus.UNCONSUMED)).toHashSet()
            }
            else HashSet()

        // Retain those in both consumed and netDelta
        val unconsumedStateRefs = Sets.union(netDelta.produced, unconsumedStatesOfType).map { it.ref }.toHashSet()
        unconsumedStateRefs.retainAll(unconsumedStatesOfType.map { it.ref }.toHashSet())
        // Retain those consumed by txn inputs
        txnConsumedStates.retainAll(txnConsumedStates.filter { unconsumedStateRefs.contains(it.ref) })

        // Is transaction irrelevant?
        if (txnConsumedStates.isEmpty() && ourNewStates.isEmpty()) {
            log.trace { "tx ${tx.id} was irrelevant to this vault, ignoring" }
            return Vault.NoUpdate
        }

        return Vault.Update(txnConsumedStates, ourNewStates.toHashSet())
    }
